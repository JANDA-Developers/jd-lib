...
export const PasswordReseter: React.FC<IProps> = ({
    langs = DEFAULT_LANGS,
    startChangeCallBack,
    ...
    const validate = () => {
        if (requireField?.password && newPasswordHook.value !== newConfimPasswordHook.value) {
    ...
    return (
        <JDmodal
            ...
            {...props}
        >
            {requireField?.email && <div>
                <InputText id="veriEmialInput" label={langs["eamil"]} {...emailHook} />
            </div>}
            <Align flex>
                <InputText
                    id="veriPhoneInput"
